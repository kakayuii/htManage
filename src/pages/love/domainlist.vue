      ...
      <el-table :data="showList" border
        ...
        <el-table-column label="操作" width="190">
          <template slot-scope="scope">
            ...
            <i class="el-icon-caret-top "   v-if="scope.row.numberIndex>0&&keyword==''" @click="upOrDownDomain(0,scope.row.orderId)"></i>
            <i class="el-icon-caret-bottom"  v-if="scope.row.numberIndex!=total&&keyword==''" @click="upOrDownDomain(1,scope.row.orderId)"></i>
            <!--<i class="el-icon-caret-bottom"  v-if="!(scope.$index==list.length%Page&&Page==Math.ceil(list.length/PageSize))" @click="upOrDownDomain(1,scope.row.orderId)"></i>-->
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
...
<script>
  ...
  export default  {
    ...
    methods: {
      async getList(){
        this.loading = true;
        let res = await API.love.DomainList();
  ...
  upOrDownDomain(type,domainId){

      API.love.upOrDownDomain({type:type,domainId:domainId}).then(res=>{
