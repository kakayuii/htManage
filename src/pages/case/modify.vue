<template>
  <section>
    <div v-title>案例管理{{form.pictureId}}</div>
    ...
    <el-card>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="案例内容" prop="caseContent">
          <editor ref="editor" class="editor" :msg="form.caseContent">{{form.caseContent}}</editor>
        </el-form-item>
        <el-form-item label="关键字" prop="keyWords">
          <el-input v-model="form.keyWords"></el-input>
        </el-form-item>
        <el-form-item label="标题" prop="caseTitle">
          <el-input v-model="form.caseTitle"></el-input>
        </el-form-item>
...
<script>
  ...
  export default class CaseModify extends BaseComponent {
    ...
    onSubmit(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          this.form.caseContent = this.$refs.editor.getUEContent();
          let res = await API.case.saveCase(this.form);
          await this.$message("编辑成功");
          this.$router.push({name: 'caselist', params: {}})
        } else {
          return false;
        }
      });
    ...
    async getDate(id){
      let res = await API.case.getCase(id);
      this.form = res;
