<template>
  <section>
    ...
    <el-card class="search-box">
      <el-input placeholder="请输入搜索内容" style="width:360px;" v-model.trim="search.keyword" @keyup.native="searching()"></el-input>
    ...
    <el-card>
      <el-table :data="showList" border
        ...
        <el-table-column prop="releaseFlag" label="是否发布" width="110">
...
<script>
  ...
  export default class NewsList extends BaseComponent {
    ...
    async getList(){
      this.loading = true;
      let res = await API.news.ArticleList();
      this.loading = false;
      this.date = res;
      this.searching();
    }
    ...
    gotoAdd(){
      this.$router.push({name: 'newsadd', params: {}})
    }
    gotoModify(id){
      this.$router.push({name: 'newsmodify', params: {id:id}})
    }
    ...
    delDomain(id){
      this.$confirm('确认删除吗?', '提示', {
        ...
        let res = await API.news.delArticle({id:id});
    ...
    searching(){
      ...
      if(this.date.length > 0 && this.search.keyword !='') {
        ...
        for(let t =0;t<this.date.length;t++) {
          ...
          if(search(item.articleTitle,this.search.keyword))this.list.push(item);
        }
      }else{
        this.list = this.date;
      }
