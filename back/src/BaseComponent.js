...
export default class BaseComponent extends Vue {
  ...
  getmodPath() {
    ...
    sessionStorage['modPath']=this.modPath;
  }
  ...
  setPageCache(page,val) {
    //store.set(page,val);
    sessionStorage[page]=JSON.stringify(val);
  }

  getPageCache(page) {
    var val=null;
    if(sessionStorage[page]!="-1"&&sessionStorage[page]){
      val=JSON.parse(sessionStorage[page]);
    }
